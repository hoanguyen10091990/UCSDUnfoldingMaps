...
public class EarthquakeCityMap extends PApplet {
	...
	private void addKey() {	
		...
		// Add key for map marker
		fill(217, 0, 0);
		triangle(50, 125, 70, 125, 60, 110);
		fill(255, 255, 255);
		...
		fill(color(0, 0, 255));
		ellipse(60, 245, 15, 15);
		fill(color(255, 0, 0));
		ellipse(60, 270, 15, 15);
		
		// Add explaination for key
		fill(0, 0, 0);
	...
	private void printQuakes() 
	{
		...
		for (Marker country: countryMarkers) {
			...
			for (Marker marker : quakeMarkers) {
				...
				if (em.isOnLand() && em.getProperty("country").equals(name)) {
					counter++;
				}
			}
			// Print country and earthquake number if at least 1 earthquake occur in this country
			if (counter > 0) {
			System.out.println(name + ": " + counter);
			}
			totalLandQuake += counter;
		}
		numOceanQuakes = quakeMarkers.size() - totalLandQuake;
